...
func TestDocsListCommandUsesDriveListEndpoint(t *testing.T) {
	...
	out := buf.String()
	if !strings.Contains(out, "d1\tDoc\tdocx\thttps://example.com/doc") {
		t.Fatalf("unexpected output: %q", out)
	}
	if strings.Contains(out, "s1\tSheet\tsheet") {
		t.Fatalf("expected sheet to be filtered out, got: %q", out)
	}
}
