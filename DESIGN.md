...
| Users get | `/open-apis/contact/v3/users/:user_id` | SDK contact | tenant | v3 | `lark users get`, `lark contacts users get`. |
...
| Drive metadata | `/open-apis/drive/v1/files/:file_token` | Core ApiReq wrapper | tenant/user | v1 | `lark drive get` / `lark drive urls`. |
...
| Sheets metadata | `/open-apis/sheets/v2/spreadsheets/:token/metainfo` | Core ApiReq wrapper | tenant/user | v2 | `lark sheets metadata`. |
...
| Meetings get | `/open-apis/vc/v1/meetings/:meeting_id` | Core ApiReq wrapper | tenant/user | v1 | `lark meetings get`. |
| Minutes get | `/open-apis/minutes/v1/minutes/:minute_token` | SDK minutes | tenant | v1 | `lark minutes get`. |
...
| Mail get | `/open-apis/mail/v1/user_mailboxes/:mailbox_id/messages/:message_id` | Core ApiReq wrapper | tenant/user | v1 | `lark mail get`. |
